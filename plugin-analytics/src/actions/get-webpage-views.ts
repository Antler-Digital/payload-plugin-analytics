...
import { UTMTracking } from "../utils/utm-tracking";
...
async function connectToDb(payload: BasePayload): Promise<MongoClient> {
  const dbUrl = getDbUrl(payload);
  return new MongoClient(dbUrl);
}

async function getCollection(payload: BasePayload, collection: CollectionSlug) {
  const db = await connectToDb(payload);
...
export async function getWebpageViews(
  ...
  return new Response(JSON.stringify({ value: count.totalDocs, change: 100 }), {
    headers,
  });
}
...
export async function getUniqueVisitors(
  ...
  try {
    ...
    if (aggregate.length === 0) {
      return new Response(JSON.stringify({ value: 0, change: 0 }), {
        headers,
      });
    }

    return new Response(
      JSON.stringify({ value: aggregate[0]?.session_ids, change: -10 }),
      {
        headers,
      },
    );
...
export async function getBounceRate(
  ...
  return new Response(JSON.stringify({ value: count.totalDocs, change: 23 }), {
    headers,
  });
}
...
export async function getLiveVisitors(
  ...
  return new Response(JSON.stringify({ value: count.totalDocs, change: -7 }), {
    headers,
  });
}

export async function getPageViewsAndVisitors(
  payload: BasePayload,
  collection: CollectionSlug,
) {
  const count = await payload.count({
...
export async function getVisitorGeography(
  payload: BasePayload,
  collection: CollectionSlug,
) {
  const count = await payload.count({
...
export async function getTopPages(
  ...
  all.docs.forEach((doc) => {
    uniquePaths.set(
      doc.path as string,
      (uniquePaths.get(doc.path as string) || 0) + 1,
    );
  });
  const docs = Array.from(uniquePaths.entries())
    .slice(
...
export async function getUTMTracking(
  ...
  try {
    ...
    all.docs.forEach((doc) => {
      const key = UTMTracking.utmToKey(doc.utm as any);
    ...
    const docs = Array.from(uniqueCampaigns.entries())
      .slice(
        (parseInt(opts.page || "1", 10) - 1) *
          parseInt(opts.limit || "1000", 10),
        parseInt(opts.page || "1", 10) * parseInt(opts.limit || "1000", 10),
      )
      .map(([campaign, count]) => {
...
export async function getOperatingSystems(
  ...
  try {
    ...
    all.docs.forEach((doc) => {
      uniqueBrowsers.set(
        doc.os as string,
        (uniqueBrowsers.get(doc.os as string) || 0) + 1,
      );
    });
    const docs = Array.from(uniqueBrowsers.entries())
      .slice(
